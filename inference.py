...
def get_mask(model, input_img, use_amp=True, s=640):
    ...
    tmpImg[ph // 2:ph // 2 + h, pw // 2:pw // 2 +
           w] = cv2.resize(input_img, (w, h)) / 255
    tmpImg = tmpImg.transpose((2, 0, 1))
    tmpImg = torch.from_numpy(tmpImg).unsqueeze(
        0).type(torch.FloatTensor).to(model.device)
    with torch.no_grad():
        if use_amp:
            with amp.autocast():
                pred = model(tmpImg)
        ...
        pred = cv2.resize(pred.cpu().numpy().transpose(
            (1, 2, 0)), (w0, h0))[:, :, np.newaxis]
        return pred
...
if __name__ == "__main__":
    ...
    for i, path in enumerate(tqdm(sorted(glob.glob(f"{opt.data}/*.*")))):
        ...
        if opt.only_matted:
            ...
            img = np.concatenate(
                (img, mask * img, mask.repeat(3, 2) * 255), axis=1).astype(np.uint8)
